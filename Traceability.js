...
function onEditInstalled(e) {
  try {
    ...
    if (sheetName === 'Ordenes') {
      ...
      if (colStatusTrackerIdx && startCol <= colStatusTrackerIdx && endCol >= colStatusTrackerIdx) editIntersectsRefs = true;
        ...
        if (processNumRows > 0) {
          ...
          // EDICIÓN INDIVIDUAL O PEQUEÑA:
            ...
            for (var r = 0; r < processNumRows; r++) {
              ...
              if (currStatus === "Impreso" || currStatus === "Reimpreso" || currStatus === "Anulada") {
                if (currStatus === "Anulada") {
                  shouldValidate = true; // actualizarEstadoDocumentosEnHoja se encargará de poner "🚫 Orden Anulada" rápido
                } else if (numRows === 1) { // Solo preguntar si es una sola edición
                  var ui = SpreadsheetApp.getUi();
                  var response = ui.alert(
